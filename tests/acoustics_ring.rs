...
fn staged_dispatch(gpu: &Gpu, header: &DispatchHeader, sources: &[Source]) -> (wgpu::Buffer, DispatchShape) {
...
fn l10_results_echo_their_tags_and_a_reassigned_slot_is_identifiable() {
    ...
    let header = DispatchHeader::new(&listener_at([20.0, 1.6, 60.0]), &Air::standard(), &[]).unwrap();
    ...
    let tags = [0u32, 1, 2, 0x7F_FFFF, 0x80_0000, MAX_TAG, 12_345, 999_999, 3, 4];
    let sources: Vec<Source> = tags
        .iter()
        .enumerate()
        .map(|(i, &t)| {
            let x = if i % 2 == 0 { 100.0 } else { 30.0 };
            Source::new([x, 1.0, 20.0 + 8.0 * i as f32], 1.0, [0.0; 3], t, (i as u8) * 20).unwrap()
        })
        .collect();
    ...
    ac.encode(&mut enc, Staged { buffer: &stage, offset: 0, len: shape.bytes }, shape).unwrap();
    ...
    assert_eq!(stale, vec![3, 7], "the reassigned slots are exactly the stale ones");
    // The flags ride above the tag without touching it.
    assert!(out.results()[0].blocked(), "a source behind the ridge was not blocked");
...
fn l11_a_busy_slot_is_skipped_and_counted_and_nothing_is_invalid() {
    ...
    assert_eq!(ac.encode(&mut held, staged, shape).unwrap(), Encoded::Dispatched { slot: 0, sequence: 1 });
    for (slot, sequence) in [(1, 2), (2, 3), (3, 4)] {
        let mut enc = gpu.encoder();
        assert_eq!(ac.encode(&mut enc, staged, shape).unwrap(), Encoded::Dispatched { slot, sequence });
        gpu.submit_wait(enc);
    }
    let mut out = TickResults::default();
    assert!(ac.take_ready(&mut out));
    ...
    let mut enc = gpu.encoder();
    assert_eq!(ac.encode(&mut enc, staged, shape).unwrap(), Encoded::SkippedBusy);
    assert_eq!(ac.counters().skipped_busy, 1);
    // The next free slot is used after it.
    assert_eq!(ac.encode(&mut enc, staged, shape).unwrap(), Encoded::Dispatched { slot: 1, sequence: 5 });
    gpu.submit_wait(enc);
    ...
    // An encoder dropped unsubmitted hands its slot back rather than leaking it.
    {
        ...
        assert!(matches!(ac.encode(&mut dropped, staged, shape).unwrap(), Encoded::Dispatched { slot: 2, .. }));
    }
    for _ in 0..4 {
        let mut enc = gpu.encoder();
        assert!(matches!(ac.encode(&mut enc, staged, shape).unwrap(), Encoded::Dispatched { .. }));
        gpu.submit_wait(enc);
        assert!(ac.take_ready(&mut out));
    }
    assert_eq!(ac.counters().skipped_busy, 1);
    ...
    let small = AcousticLimits { sources: 1, ..AcousticLimits::MAX };
    let mut ac1 = GpuAcoustics::new(&gpu.device, small).unwrap();
    let two = [s, s];
    let (stage2, shape2) = staged_dispatch(&gpu, &header, &two);
    let mut enc = gpu.encoder();
    let err = ac1.encode(&mut enc, Staged { buffer: &stage2, offset: 0, len: shape2.bytes }, shape2);
    assert!(matches!(err, Err(AcousticsError::Limit { what: "sources", got: 2, max: 1 })));
    assert_eq!(ac1.counters().dispatches, 0);
    gpu.submit_wait(enc);

    let error = pollster::block_on(scope.pop());
    assert!(error.is_none(), "wgpu reported a validation error: {error:?}");
}
