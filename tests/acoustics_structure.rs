...
const SOURCES: &[(&str, &str)] = &[
    ...
    ("readback.rs", include_str!("../src/gpu/acoustics/readback.rs")),
    ("records.rs", include_str!("../src/gpu/acoustics/records.rs")),
    ("shader.rs", include_str!("../src/gpu/acoustics/shader.rs")),
    ("oracle.rs", include_str!("../src/gpu/acoustics/oracle.rs")),
    ("query.wgsl", include_str!("../src/gpu/acoustics/query.wgsl")),
    ("build.wgsl", include_str!("../src/gpu/acoustics/build.wgsl")),
];
...
fn code(src: &str) -> impl Iterator<Item = (usize, &str)> {
    src.lines().enumerate().filter(|(_, l)| !l.trim_start().starts_with("//"))
}
...
fn l8_the_acoustics_never_poll_and_take_the_device_only_to_build() {
    for (name, src) in SOURCES {
        for (n, line) in code(src) {
            ...
            assert!(!l.contains(".poll("), "{name}:{}: polls the device: {line}", n + 1);
            assert!(!l.contains("poll_all"), "{name}:{}: polls the instance: {line}", n + 1);
            assert!(!l.contains("pollster"), "{name}:{}: blocks on a future: {line}", n + 1);
            assert!(!l.contains("block_on"), "{name}:{}: blocks on a future: {line}", n + 1);
            assert!(!l.contains("on_submitted_work_done"), "{name}:{}: waits on the queue: {line}", n + 1);
        }
    }
    ...
    for (name, src) in SOURCES.iter().filter(|(n, _)| n.ends_with(".rs")) {
        ...
        for (i, line) in lines.iter().enumerate() {
            ...
            let fn_name = t.split("fn ").nth(1).unwrap().split(['(', '<']).next().unwrap().to_string();
    ...
    for f in &public_takers {
        assert!(f.ends_with(": new"), "a public function other than new takes the device: {f}");
    }
