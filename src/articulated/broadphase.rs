...
impl Grid {
    ...
    pub(super) fn pairs(
        ...
        if !self.oversized.is_empty() {
            self.sweep_oversized(position, inv_mass, jointed, frontier, swept, out, reached);
        }
    }
        ...
        for &packed in self.oversized.iter() {
            ...
            self.near(position[a], self.reach[a], |b| {
                ...
                if sweeping {
                    ...
                    if !frontier.get(b) && !swept.get(b) {
                        // Nobody has looked from it and nobody will until it is woken.
                        reached.push(b);
                    }
                } else if frontier.get(b) || swept.get(b) {
                    // Asleep with something awake beside it. Waking it is all this round
